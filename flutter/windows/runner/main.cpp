...
typedef char** (*FUNC_ONEDESK_CORE_MAIN)(int*);
typedef void (*FUNC_ONEDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_ONEDESK_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libonedesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libonedesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_ONEDESK_CORE_MAIN onedesk_core_main =
      (FUNC_ONEDESK_CORE_MAIN)GetProcAddress(hInstance, "onedesk_core_main_args");
  if (!onedesk_core_main)
  {
    std::cout << "Failed to get onedesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_ONEDESK_FREE_ARGS free_c_args =
      (FUNC_ONEDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = onedesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "OneDesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  std::vector<std::string> rust_args(c_args, c_args + args_len);
  free_c_args(c_args, args_len);

  std::wstring app_name = L"OneDesk";
  FUNC_ONEDESK_GET_APP_NAME get_onedesk_app_name = (FUNC_ONEDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_onedesk_app_name");
  if (get_onedesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_onedesk_app_name(app_name_buffer, 512) == 0) {
