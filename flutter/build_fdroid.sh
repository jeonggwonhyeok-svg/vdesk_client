...
#
# Script to build F-Droid release of OneDesk
#
# Copyright (C) 2024, The OneDesk Authors
...
arm64-v8a)
	...
	ONEDESK_FEATURES='flutter,hwcodec'
	;;
armeabi-v7a)
...
prebuild)
	...
	# Flutter used to compile main Onedesk library
	...
	# Install Flutter version for OneDesk library build
...
build)
	...
	cargo ndk \
		...
		build \
		--release \
		--features "${ONEDESK_FEATURES}"

	mkdir -p "flutter/android/app/src/main/jniLibs/${ANDROID_ABI}"

	cp "target/${RUST_TARGET}/release/liblibonedesk.so" \
		"flutter/android/app/src/main/jniLibs/${ANDROID_ABI}/libonedesk.so"
