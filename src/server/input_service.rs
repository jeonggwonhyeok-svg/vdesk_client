...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in OneDesk
            // 2. Move the physical mouse
            // 3. Press "V" key in OneDesk
            ...
            // we consider only OneDesk operates the controlled side.
            //
...
fn key_sleep() {
    ...
    // Though `/Applications/OneDesk.app/Contents/MacOS/onedesk --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run onedesk from cmd
        std::thread::spawn(|| {
