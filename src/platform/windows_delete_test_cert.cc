...
//*************************************************************
//
//  DeleteOneDeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes OneDesk Test certificates and wrong key stores
//
...
BOOL DeleteOneDeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteOneDeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteOneDeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteOneDeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteOneDeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
