...
lazy_static::lazy_static! {
    ...
    static ref SUDO_E_PRESERVES_ENV: bool = {
        if !is_root() {
            ...
            let key = format!("__ONEDESK_SUDO_E_TEST_{}", std::process::id());
            let val = "1";
...
// stop_onedesk_servers() is just a temp solution here.
fn force_stop_server() {
    stop_onedesk_servers();
...
mod desktop {
    ...
    pub struct Desktop {
        ...
        pub home: String,
        pub dbus: String,
        pub is_onedesk_subprocess: bool,
    ...
    impl Desktop {
        ...
        pub fn is_headless(&self) -> bool {
            self.sid.is_empty() || self.is_onedesk_subprocess
        }
        ...
        fn set_is_subprocess(&mut self) {
            self.is_onedesk_subprocess = false;
            let cmd = format!(
            ...
            if let Ok(res) = run_cmds(&cmd) {
                if res.trim() != "0" {
                    self.is_onedesk_subprocess = true;
                }
            }
        }
...
pub fn uninstall_service(show_new_window: bool, _: bool) -> bool {
    ...
    // systemctl kill onedesk --tray, execute cp first
